...
import { Phone, Mail, MapPin, Menu, X, ChevronRight, Instagram, Facebook, Twitter, Award, CheckCircle2, Star, Play, ExternalLink, ArrowUp, MessageCircle } from 'lucide-react';
  ...
  <div className="hidden lg:block bg-dark-secondary border-b border-white/5 py-2 text-xs">
    <div className="container mx-auto px-4 flex justify-between items-center">
      ...
      <div className="flex items-center gap-6">
        <a href="mailto:Info@shreekhatushyammotors.co.in" className="flex items-center gap-2 hover:text-primary transition-colors">
          <Mail className="w-3 h-3 text-primary" />
          Info@shreekhatushyammotors.co.in
        </a>
        <a href="tel:8218258915" className="flex items-center gap-2 hover:text-primary transition-colors">
...
export const Navbar = () => {
  ...
  return (
    <nav className={cn(
      ...
      isScrolled ? "bg-dark/80 backdrop-blur-lg py-3 shadow-xl" : "bg-transparent py-5"
    )}>
      <div className="container mx-auto px-4 flex justify-between items-center">
        <Link to="/" className="flex items-center gap-2 group">
          <img src={LOGO_SRC} alt="Shree Khatu Shyam Motors logo" className="w-10 h-10 rounded-lg object-contain bg-white/5 group-hover:rotate-12 transition-transform" referrerPolicy="no-referrer" />
          <div className="flex flex-col">
            <span className="font-display font-bold text-lg leading-none tracking-tight">SHREE KHATU SHYAM</span>
            <span className="text-primary font-bold text-sm tracking-[0.2em] leading-none">MOTORS</span>
          </div>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <Link to="/contact" className="bg-primary hover:bg-primary-light text-white px-5 py-2 rounded-full text-sm font-semibold transition-all hover:scale-105">
            Book Service
          </Link>
        </div>
            ...
            >
              <div className="flex justify-between items-center mb-12">
                <div className="flex items-center gap-2">
                  <img src={LOGO_SRC} alt="Logo" className="w-8 h-8 rounded-xl object-contain" referrerPolicy="no-referrer" />
                  <span className="font-display font-bold text-xl">SKS MOTORS</span>
                </div>
                <button 
              ...
              <div className="flex flex-col gap-4">
                ...
                >
                  <Link
                    to="/contact"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="w-full bg-primary text-white py-4 rounded-2xl text-center font-bold text-lg shadow-lg shadow-primary/20 flex items-center justify-center gap-2"
                  >
                    Book Service
                  </Link>
                </motion.div>
              ...
              <div className="mt-auto pt-8 border-t border-white/5">
                <p className="text-gray-500 text-xs mb-4 uppercase tracking-widest font-bold">Follow Us</p>
                <div className="flex gap-4">
                  {[Facebook, Instagram, Twitter].map((Icon, i) => (
                    <a key={i} href="#" className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center hover:bg-primary transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
        <div className="space-y-6">
          <Link to="/" className="flex items-center gap-2">
            <img src={LOGO_SRC} alt="Logo" className="w-8 h-8 rounded-xl object-contain" referrerPolicy="no-referrer" />
            <div className="flex flex-col">
              <span className="font-display font-bold text-xl leading-none">SHREE KHATU SHYAM</span>
              <span className="text-primary font-bold text-xs tracking-[0.2em]">MOTORS</span>
            </div>
          </Link>
          <p className="text-gray-400 text-sm leading-relaxed">
            Reliable Transport & Trusted Services in Mathura. Delivering excellence with faith and trust since 2026.
          </p>
          <div className="flex gap-4">
            {[Facebook, Instagram, Twitter].map((Icon, i) => (
        ...
        <div>
          ...
          <ul className="space-y-4 text-gray-400 text-sm">
            {['Home', 'About', 'Services', 'Gallery', 'Certificates', 'Contact'].map((item) => (
              <li key={item}>
                <Link to={item === 'Home' ? '/' : `/${item.toLowerCase()}`} className="hover:text-primary transition-colors flex items-center gap-2">
        ...
        <div>
          ...
          <ul className="space-y-4 text-gray-400 text-sm">
            {['Transportation', 'House Cleaning', 'Logistics', 'Goods Relocation'].map((item) => (
        ...
        <div>
          ...
          <ul className="space-y-4 text-gray-400 text-sm">
            <li className="flex gap-3">
              ...
              <span>Ground Floor, Service Rd, nearby 1 Mandi Chauraha, Surya Nagar, Mathura, UP 281004</span>
            </li>
            <li className="flex gap-3">
